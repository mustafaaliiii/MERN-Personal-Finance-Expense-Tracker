...
const Profile = () => {
  ...
  const saveCurrencyPref = async () => {
    try {
      // Set currency to PKR
      setCurrency("PKR");
      toast.success("Currency set to PKR");
      // Charts will update automatically via currencyChanged event listener
    } catch (e) {
      console.error(e);
          ...
          <div className="card dark:bg-slate-800 dark:border-slate-700 flex items-center justify-between flex-col">
            ...
            {userProfile?.profileImageUrl ? (
              <img
                ...
                alt="Profile"
                className="w-42 h-42 bg-slate-400 rounded-full object-cover"
              />
            ) : (
              <CharAvatar
