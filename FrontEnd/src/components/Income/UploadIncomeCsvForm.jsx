...
const UploadIncomeCsvForm = ({ onSuccess, onCancel }) => {
  ...
  const handleUploadAndParse = async () => {
    ...
    setLoading(true);
    try {
      const res = await axiosInstance.post(API_PATHS.INCOME.PARSE_CSV, formData, {
  ...
  const handleSourceChange = (id, newSrc) => {
    setParsedData(prev => prev.map(item => item.id === id ? { ...item, source: newSrc } : item));
  };

  const handleSaveBulk = async () => {
    setLoading(true);
    try {
      await axiosInstance.post(API_PATHS.INCOME.BULK_ADD, { incomes: parsedData });
      toast.success("All incomes saved successfully!");
      onSuccess();
    } catch (error) {
      console.error(error);
      toast.error("Failed to save incomes");
    } finally {
      setLoading(false);
    }
  };
    ...
    return (
      <div className="flex flex-col gap-4 py-4">
        <p className="text-sm text-slate-600 dark:text-slate-300">
          Upload your bank statement or incoming payments file (<b>.pdf</b>, .csv, or .xlsx). The system will automatically extract the dates, amounts, and descriptions!
        </p>
        <input 
          type="file" 
          accept=".csv,.xlsx,.pdf" 
  ...
  return (
    <div className="flex flex-col gap-4 py-2">
      <p className="text-sm text-amber-600 dark:text-amber-400 font-medium">
        Review your incoming transactions before saving.
      </p>
        ...
        <table className="w-full text-left text-sm whitespace-nowrap">
          <thead className="bg-slate-100 dark:bg-slate-800 sticky top-0">
            <tr>
              ...
              <th className="px-4 py-2 font-medium">Source / Description</th>
          ...
          <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
            {parsedData.map((item) => (
              <tr key={item.id}>
                ...
                <td className="px-4 py-3 max-w-xs truncate" title={item.source}>{item.source}</td>
      ...
      <div className="flex justify-end gap-3 mt-4">
        ...
        <button className="px-4 py-2 bg-primary text-white rounded-md text-sm cursor-pointer" onClick={handleSaveBulk} disabled={loading || parsedData.length === 0}>
          {loading ? "Saving..." : `Save ${parsedData.length} Incomes`}
        </button>
      </div>
    </div>
  );
...
export default UploadIncomeCsvForm;
